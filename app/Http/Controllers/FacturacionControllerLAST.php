...
class FacturacionController extends Controller
{
    ...
    public function data_print_invoice($nro_factura = Null,$tipo_documento = Null)
    {
        ...
        // if ($ventas->count() == 2) {
        //     $total = $venta->montototal + $ventas->last()->montototal;
        ...
        return compact('total', 'empresa1', 'empresa2', 'detalles1',  'nro_factura', 'cliente','venta','subtotaliva12','subtotaliva0');
    ...
    public function search_product_code(Request $request)
    {
        ...
        if ($product == Null) {
            ...
            return response()->json(['result' => 'ok', 'data' => $product[0]]);
        }
    }
    ...
    public function pay(Request $request)
    {
        ...
        $venta->montoiva = $totalf * 0.12; //preguntar	
        ...
        foreach ($request->productos as $p) {
            ...
            $detalle->precio = $p['cantidad'];
            $detalle->subtotal = $p['totalsiniva'] * $p['cantidad']; //preguntar
            $detalle->iva = 1;
            $detalle->montoiva = $p['total'];
            $detalle->descmonto = $p['descmonto'];
            $detalle->descsubtotal = ($p['pro_precioventa'] * $p['cantidad']) - $p['descmonto'];
